...
import { X, Clock, ArrowRight, Target } from "lucide-react";
...
export function FloatingWebinarWidget({ onOpenDiagnostic }: FloatingWebinarWidgetProps) {
  ...
  return (
    <div className="fixed bottom-3 right-3 sm:bottom-5 sm:right-5 z-40 origin-bottom-right flex flex-col items-end pointer-events-none">
      
      {/* 1. Minimized Pill Button */}
      <div
        className={
          "transition-all duration-200 ease-out origin-bottom-right " +
          (isMinimized
            ? "opacity-100 scale-100 translate-y-0 pointer-events-auto relative"
            : "opacity-0 scale-90 translate-y-2 pointer-events-none absolute bottom-0 right-0")
        }
      >
        <button
          type="button"
          onClick={() => setIsMinimized(false)}
          className="flex items-center gap-2 rounded-full border-2 border-[#B64F43] bg-[#0C263D] px-4 py-2.5 text-xs font-extrabold uppercase tracking-wider text-white shadow-2xl hover:bg-[#1A4368] transition-all cursor-pointer whitespace-nowrap"
        >
      ...
      {/* 2. Expanded Floating Assessment Card */}
      <div
        className={
          "w-[calc(100vw-1.5rem)] max-w-[350px] rounded-2xl border border-white/20 bg-[#0C263D] text-white shadow-2xl overflow-hidden origin-bottom-right transition-all duration-200 ease-out " +
          (!isMinimized
            ? "opacity-100 scale-100 translate-y-0 pointer-events-auto relative"
            : "opacity-0 scale-90 translate-y-4 pointer-events-none absolute bottom-0 right-0")
        }
      >
        {/* Top Banner with Dan's Photo & Minimize Button */}
